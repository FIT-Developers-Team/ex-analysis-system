...
export function runSimulation(baseline: SimulationBaseline, inputs: SimulationInputs): SimulationResult {
  ...
  const fulfillmentChange = inputs.processGain * 0.22 + inputs.attendanceChange * 0.16 - inputs.forecastChange * 0.12 - cancellationLoad * 0.08;
  const utilizationChange = baseline.utilization * (demandFactor - 1) - inputs.processGain * 0.08;
  const mandaysGapChange = baseline.mandaysGap + inputs.attendanceChange;
  ...
  if (!notes.length) notes.push("Skenario berada dalam rentang operasi terkendali; pantau SLA dan fulfillment sebagai guardrail.");

  return {
    productivityChange: clamp(productivityChange, -40, 40),
    slaChange: clamp(slaChange, -15, 15),
    fulfillmentChange: clamp(fulfillmentChange, -12, 12),
