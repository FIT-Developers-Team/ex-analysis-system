...
function OperatingPictureBrief({ data }: { data: AnalysisPayload }) {
  ...
  return (
    <section className={`operating-picture operating-picture--${picture.mode}`} aria-labelledby="operating-picture-title">
      <header>
        <div><span className="eyebrow">Bayangan operasi aktual</span><h2 id="operating-picture-title">{picture.headline}</h2></div>
        <div className="operating-picture__status"><strong>{picture.label}</strong><span>Keyakinan {picture.confidence}</span></div>
      </header>
      <p className="operating-picture__situation">{picture.situation}</p>
      <div className="operating-signature" aria-label="Operational state signature">{picture.signature.length ? picture.signature.map((item) => <span key={item}>{item.replaceAll("_", " ")}</span>) : <span>NO DOMINANT BREACH</span>}</div>
      <div className="operating-picture__evidence">
        <section><strong>Fakta yang terbaca</strong>{picture.verifiedFacts.map((item) => <p key={item}><CheckCircle2 size={14} />{item}</p>)}</section>
        <section><strong>Mekanisme yang masuk akal</strong>{picture.plausibleMechanisms.map((item) => <p key={item}><GitBranch size={14} />{item}</p>)}</section>
        <section><strong>Jangan langsung simpulkan</strong>{picture.alternativeExplanations.map((item) => <p key={item}><Info size={14} />{item}</p>)}</section>
      </div>
      <div className="decision-sequence" aria-label="Urutan keputusan">
        {picture.decisionSequence.map((step, index) => <article key={`${step.phase}-${step.owner}`}><span>0{index + 1} · {step.phase}</span><strong>{step.action}</strong><p>{step.owner}</p><small>Gate: {step.exitGate}</small></article>)}
      </div>
  ...
  return (
    <>
      ...
      <section className="section-block operational-thread-section">
        <SectionHeader eyebrow="End-to-end operating threads" title="Lihat chain yang terhubung, tertekan, atau terputus" description="Setiap jalur menyatukan plan, workload, manpower, proses, service, dan value. Tahap kosong ditampilkan sebagai gap keputusan—bukan dianggap normal." />
        <div className="operational-thread-grid">
          {data.operationalThreads.map((thread) => (
            <article className={`operational-thread operational-thread--${thread.state}`} key={thread.id}>
              ...
              <div className="thread-stage-rail">
      ...
      <section className="initiative-grid">
        {data.initiatives.map((initiative, index) => (
          <article className="initiative-card" key={initiative.id}>
            <header><span className="initiative-number">0{index + 1}</span><div><div className="tag-row"><span>{initiative.portfolioRole}</span><span>{initiative.type}</span><span>keyakinan {initiative.confidence}</span><span>prioritas {initiative.priorityScore}</span></div><h3>{initiative.title}</h3><div className="initiative-meta"><span><Users size={13} />{initiative.owner}</span><span><BarChart3 size={13} />usaha {initiative.effort}</span><span><CalendarDays size={13} />{initiative.horizonDays} hari</span></div></div></header>
            ...
            <div className="initiative-experiment"><div><span>Pertanyaan keputusan</span><strong>{initiative.decisionQuestion}</strong></div><div><span>Apa yang membantah hipotesis</span><p>{initiative.counterfactual}</p></div><div><span>Leading indicators</span><p>{initiative.leadingIndicators.join(" · ")}</p></div></div>
  ...
  return (
    <>
      ...
      <section className="section-block context-gap-section">
        <SectionHeader eyebrow="Decision coverage backlog" title="Kekosongan mana yang paling mengubah keputusan" description="Prioritas mempertimbangkan dampak bisnis, chain yang terputus, dan apakah gap tersebut menghalangi diagnosis manpower, service, capacity, quality, atau cost." />
        <div className="context-gap-grid">
          ...
          {!data.contextGaps.length && <div className="panel empty-state"><CheckCircle2 size={18} /><p>Tidak ada context gap material pada window aktif.</p></div>}
        </div>
      </section>
